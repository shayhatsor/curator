...
//ORIGINAL LINE: public static String validatePath(String path) throws IllegalArgumentException
		public static string validatePath(string path)
		{
			if (path == null)
			{
				throw new System.ArgumentException("Path cannot be null");
			}
			if (path.Length == 0)
			{
				throw new System.ArgumentException("Path length must be > 0");
			}
			if (path[0] != '/')
			{
				throw new System.ArgumentException("Path must start with / character");
			}
			if (path.Length == 1)
			...
			{
				throw new System.ArgumentException("Path must not end with / character");
			}
			...
			if (reason != null)
			{
				throw new System.ArgumentException("Invalid path string \"" + path + "\" caused by " + reason);
			}
