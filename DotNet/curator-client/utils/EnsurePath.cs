...
namespace org.apache.curator.utils
{
	...
	/// </para>
	/// <para>
	/// The first time it is used, a synchronized call to <seealso cref="ZKPaths#mkdirs(ZooKeeper, String)"/> is made to
	...
	[Obsolete("Since 2.9.0 - Prefer CuratorFramework.create().creatingParentContainersIfNeeded() or CuratorFramework.exists().creatingParentContainersIfNeeded()")]
	public class EnsurePath
	{
		...
		{
			public HelperAnonymousInnerClassHelper()
			{
			}
...
//ORIGINAL LINE: public void ensure(org.apache.curator.CuratorZookeeperClient client) throws Exception
		...
		protected internal EnsurePath(string path, AtomicReference<Helper> helper, bool makeLastNode, InternalACLProvider aclProvider)
		{
			this.path = path;
		...
		private class InitialHelper : Helper
		{
			...
			internal bool isSet = false; // guarded by synchronization
...
//JAVA TO C# CONVERTER WARNING: 'final' parameters are not available in .NET:
			public virtual void ensure(CuratorZookeeperClient client, string path, bool makeLastNode)
			{
				lock (this)
				{
					if (!isSet)
					{
						RetryLoop.callWithRetry(client, new CallableAnonymousInnerClassHelper(this, client, path, makeLastNode));
					}
				}
			}
			...
			private class CallableAnonymousInnerClassHelper : Callable<object>
			{
				...
				private string path;
				private bool makeLastNode;

				public CallableAnonymousInnerClassHelper(InitialHelper outerInstance, CuratorZookeeperClient client, string path, bool makeLastNode)
				{
					this.outerInstance = outerInstance;
					this.client = client;
					this.path = path;
...
//ORIGINAL LINE: @Override public Object call() throws Exception
				...
				{
					ZKPaths.mkdirs(client.getZooKeeper(), path, makeLastNode, outerInstance.outerInstance.aclProvider, outerInstance.outerInstance.asContainers());
