...
namespace org.apache.curator
{
	...
	/// <summary>
	/// <para>
	///     See <seealso cref="RetryLoop"/> for the main details on retry loops. <b>All Curator/ZooKeeper operations
	...
	/// </pre>
	/// </summary>
	public class SessionFailRetryLoop : System.IDisposable
	{
		...
		{
			public WatcherAnonymousInnerClassHelper()
			{
			}
		...
		private static readonly ISet<Thread> failedSessionThreads = Sets.newSetFromMap(Maps.newConcurrentMap<Thread, bool?>());
		...
		{
			/// <summary>
			/// If the session fails, retry the entire set of operations when <seealso cref="SessionFailRetryLoop#shouldContinue()"/>
			/// is called
			/// </summary>
			RETRY,
...
//ORIGINAL LINE: public void takeException(Exception exception) throws Exception
		public virtual void takeException(Exception exception)
		{
			...
			if (sessionHasFailed.get())
			{
				switch (mode)
				{
					case org.apache.curator.SessionFailRetryLoop.Mode.RETRY:
					{
					...
					case org.apache.curator.SessionFailRetryLoop.Mode.FAIL:
					{
						break;
					}
				}
