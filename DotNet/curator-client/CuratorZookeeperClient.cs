...
namespace org.apache.curator
{
	...
	using DefaultTracerDriver = org.apache.curator.utils.DefaultTracerDriver;
	using DefaultZookeeperFactory = org.apache.curator.utils.DefaultZookeeperFactory;
...
//ORIGINAL LINE: @SuppressWarnings("UnusedDeclaration") public class CuratorZookeeperClient implements java.io.Closeable
	public class CuratorZookeeperClient : System.IDisposable
	{
		...
		private readonly AtomicReference<TracerDriver> tracer = new AtomicReference<TracerDriver>(new DefaultTracerDriver());
		...
		public CuratorZookeeperClient(string connectString, int sessionTimeoutMs, int connectionTimeoutMs, Watcher watcher, RetryPolicy retryPolicy) : this(new DefaultZookeeperFactory(), new FixedEnsembleProvider(connectString), sessionTimeoutMs, connectionTimeoutMs, watcher, retryPolicy, false)
		{
		}
		...
		/// <param name="watcher"> default watcher or null </param>
		/// <param name="retryPolicy"> the retry policy to use </param>
		public CuratorZookeeperClient(EnsembleProvider ensembleProvider, int sessionTimeoutMs, int connectionTimeoutMs, Watcher watcher, RetryPolicy retryPolicy) : this(new DefaultZookeeperFactory(), ensembleProvider, sessionTimeoutMs, connectionTimeoutMs, watcher, retryPolicy, false)
		{
		}
		...
		///                      for details </param>
		public CuratorZookeeperClient(ZookeeperFactory zookeeperFactory, EnsembleProvider ensembleProvider, int sessionTimeoutMs, int connectionTimeoutMs, Watcher watcher, RetryPolicy retryPolicy, bool canBeReadOnly)
		{
			if (sessionTimeoutMs < connectionTimeoutMs)
			{
				log.warn(string.Format("session timeout [{0:D}] is less than connection timeout [{1:D}]", sessionTimeoutMs, connectionTimeoutMs));
			}
...
//ORIGINAL LINE: public void start() throws Exception
		public virtual void start()
		{
			...
			{
				System.InvalidOperationException ise = new System.InvalidOperationException("Already started");
				throw ise;
			}
...
//ORIGINAL LINE: final java.util.concurrent.CountDownLatch latch = new java.util.concurrent.CountDownLatch(1);
				...
				long elapsed = Math.Max(1, DateTimeHelperClass.CurrentUnixTimeMillis() - startTimeMs);
				waitTimeMs -= elapsed;
			}
		}
