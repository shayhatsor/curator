...
namespace org.apache.curator
{
	...
	internal class ConnectionState : Watcher, System.IDisposable
	{
		...
		internal ConnectionState(ZookeeperFactory zookeeperFactory, EnsembleProvider ensembleProvider, int sessionTimeoutMs, int connectionTimeoutMs, Watcher parentWatcher, AtomicReference<TracerDriver> tracer, bool canBeReadOnly)
		{
...
//ORIGINAL LINE: private synchronized void checkTimeouts() throws Exception
		private void checkTimeouts()
		{
			lock (this)
			{
				...
				long elapsed = DateTimeHelperClass.CurrentUnixTimeMillis() - connectionStartMs;
				if (elapsed >= minTimeout)
				{
					...
					else
					{
						...
						if (elapsed > maxTimeout)
						{
							...
							{
								log.warn(string.Format("Connection attempt unsuccessful after {0:D} (greater than max timeout of {1:D}). Resetting connection and trying again with a new connection.", elapsed, maxTimeout));
							}
							reset();
						}
						else
						{
							...
							if (!bool.getBoolean(DebugUtils.PROPERTY_DONT_LOG_CONNECTION_ISSUES))
							{
								log.error(string.Format("Connection timed out for connection string ({0}) and timeout ({1:D}) / elapsed ({2:D})", zooKeeper.getConnectionString(), connectionTimeoutMs, elapsed), connectionLossException);
							}
